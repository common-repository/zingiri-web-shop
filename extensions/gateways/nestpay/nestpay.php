...
class nestpayGateway {
	function nestpayGateway($payment,$customer) {
	...
	function calcHash() {
		$this->rnd=time();
		$hashstr = $this->payment->merchantid . $this->payment->webid . $this->payment->total . $this->payment->returnUrl . $this->payment->cancelUrl . "Auth" . "" . $this->rnd . $this->payment->secret;
		$this->hash = base64_encode(pack('H*',sha1($hashstr))); // hash value
		return $this->hash;
	}

	function replace($payment_code) {
		$payment_code = str_replace("%random%", $this->rnd, $payment_code);
		$payment_code = str_replace("%countryiso%", $this->customer['COUNTRY'], $payment_code);
		return $payment_code;
	}
}
?>
